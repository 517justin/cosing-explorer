...
| 屬名解析率 | > 98% | **99.35%** | ✅ 達標 |
| 兩路徑一致率 | 96.7% | **97.8%** | 解析器改良後提升 |
| 待人工裁決的矛盾 | 115 | **25** | 錯字先修 + 解析器改良後大幅收斂 |
...
- **同物異名對**（約 14 筆，交給 Phase 2 的 GBIF 判定即可）：
  `Centaurium erythraea`/`Erythraea centaurium`、`Lavandula latifolia`/`L. spica`（互相對調，4 筆）、`Mentha spicata`/`M. viridis`、`Perilla frutescens`/`P. ocymoides`、`Pogostemon cablin`/`P. patchouli`、`Eugenia caryophyllus`/`E. caryophyllata`
- **真正的內容矛盾**（約 11 筆，須人工確認何者為真）：
  `ROSA RUGOSA` vs `Rosa rubiginosa`（4 筆）、`CITRUS SINENSIS` vs `Citrus aurantium`（3 筆，且有一筆方向相反）、`Cinnamomum camphora` vs `C. tzumu`、`Copaifera reticulata` vs `C. officinalis`、`Elaeis oleifera` vs `E. guineensis`、`Boswellia carterii` vs `B. sacra`
...
1. 裁決 `conflicts.csv` 的 25 筆（優先：`ROSA RUGOSA` vs `Rosa rubiginosa` 4 筆）
2. 進 Phase 2 寫 `04_gbif.py`，優先解決 `family_needs_gbif = true` 的 67 筆
