...
1. ~~裁決 `conflicts.csv`~~ ✅ 已完成（4 筆更正、8 筆排除）
2. Phase 2：擴大 GBIF 查詢至全部約 1,700 個物種，取 accepted name 與完整階層；優先解決 `family_needs_gbif = true` 的 67 筆（Liliaceae 53、Scrophulariaceae 13、Myoporaceae 1）
3. Phase 3：部位與製程抽取（`05_parts.py`）
4. Phase 4 之後可平行進行
